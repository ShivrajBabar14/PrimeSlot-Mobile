...
class _LoginScreenState extends State<LoginScreen> { 
  ...
  Future<void> _getOtp() async { 
    ...
    try { 
      ...
      print('Check User API: ${checkUserResponse.statusCode} ${checkUserResponse.body}'); 
      ...
      if (checkUserResponse.statusCode == 200) { 
        ...
        if (responseData['ok'] == true && responseData['exists'] == true) { 
          ...
          try { 
            ...
            // try to detect request id / demo / messages 
            final reqId = sdkResp?['reqId'] ?? 
                sdkResp?['request_id'] ?? 
                sdkResp?['requestId'] ?? 
                sdkResp?['req_id'] ?? 
                sdkResp?['txnId'] ?? 
            ...
            final isDemo = sdkResp?['isDemo'] == true || 
                sdkResp?['demo'] == true || 
          ...
          ScaffoldMessenger.of(context).showSnackBar( 
            const SnackBar(content: Text('Phone not registered. Please sign up.')), 
          ); 
        } 
      } else { 
        ScaffoldMessenger.of(context).showSnackBar( 
